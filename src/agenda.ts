...
//  TIME_FORMAT,
//} from './constants';
//import { getSettings } from './db/api/settings/settingsApi';
//import {
...
//          // vibrate,
//        },
//        types: { events, meetings, todos, routines },
//        subscriptions,
//      },
...
//
//          Object.keys(usersRoutines)
//            .map(userId => usersRoutines[userId])
//            .forEach(list => {
//              list.forEach(async routine => {
//                const title = routine.fields.find(
//                  ({ fieldId }) => fieldId === 'TITLE',
//                ).value.text;
//                const action = routine.fields.find(
//                  ({ fieldId }) => fieldId === 'ACTION',
//                ).value.text;
//                const cycleValue = routine.fields.find(
//                  ({ fieldId }) => fieldId === 'ACTION',
//                ).value;
//                const notificationsValue = routine.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.NOTIFICATIONS,
//                ).value;
//                const notification = {
//                  body: `
//              action: ${action} |\n
//              additional note: ${notificationsValue.childrenValue &&
//                notificationsValue.childrenValue.ownValue.text}
//            `,
//                };
//
//                const notificationAt = calculateNotificationAt(
//                  TASK_TYPE.ROUTINE,
//                  routine.lastNotificationAt,
//                  cycleValue,
//                );
//
...
//
//          const usersTodosData = Object.keys(usersTodos)
//            .map(userId => usersTodos[userId])
//            .map(list =>
//              list.map(todo => {
//                const title = todo.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.TITLE,
//                ).value.text;
//                const note = todo.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.NOTE,
//                ).value.text;
//                const status = todo.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.STATUS,
//                ).value.id;
//                const notificationsValue = todo.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.NOTIFICATIONS,
//                ).value;
//
//                return {
...
//                  additionalNote:
//                    notificationsValue.childrenValue &&
//                    notificationsValue.childrenValue.ownValue.text,
//                };
//              }),
//            );
...
//                    ({ fieldId }) => fieldId === FIELD_ID.NOTE,
//                  ).value.text;
//                  const location = todo.fields.find(
//                    ({ fieldId }) => fieldId === FIELD_ID.LOCATION,
//                  ).value.text;
//                  const duration = todo.fields.find(
//                    ({ fieldId }) => fieldId === FIELD_ID.DURATION,
//                  ).value.text;
//                  const dateTime = todo.fields.find(
//                    ({ fieldId }) => fieldId === FIELD_ID.DATE_TIME,
//                  ).value.text;
//                  const notificationsValue = todo.fields.find(
//                    ({ fieldId }) => fieldId === FIELD_ID.NOTIFICATIONS,
//                  ).value;
...
//
//          userIds
//            .map(userId => usersMeetings[userId])
//            .forEach(list => {
//              list.forEach(async meeting => {
//                const title = meeting.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.TITLE,
//                ).value.text;
//                const note = meeting.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.NOTE,
//                ).value.text;
//                const person = meeting.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.PERSON,
//                ).value.text;
//                const location = meeting.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.LOCATION,
//                ).value.text;
//                const dateTimeValue = meeting.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.DATE_TIME,
//                ).value;
//                const notificationsValue = meeting.fields.find(
//                  ({ fieldId }) => fieldId === FIELD_ID.NOTIFICATIONS,
//                ).value;
